...
- **Scope is the live session list** — totals fold the list mirror the runtime keeps, so sessions outside it (archived or unloaded workspaces) do not count. The trend chart folds per-session totals at their real `updatedAt` positions; the time window filters and re-scales that fold, with no per-day bucketing or within-session series.
